...
sh::BlueShape::BlueShape(sf::Texture* texture, sf::Vector2f position)
{

	_limits[0].push_back(100);
	_limits[0].push_back(450);
	_limits[0].push_back(1000);
	_limits[1].push_back(0);
	_limits[1].push_back(400);
	_limits[1].push_back(1000);
	_limits[2].push_back(0);
	_limits[2].push_back(350);
	_limits[2].push_back(950);
	_limits[3].push_back(50);
	_limits[3].push_back(450);
	_limits[3].push_back(900);


	_position = position; 
	extraPos = sf::Vector2f(625, 800); 
...
void sh::BlueShape::rotate(sh::ShapeComponent* grid[10][20])
{
	if (_rotation == 1 && _position.x == 400) return; 
	if (_rotation == 2 && _position.x == 0) return;
	if (_rotation == 0 && _position.x == 450) return;
	if (_rotation == 3 && _position.x == 50) return; 
